...
from ...parser.json_parser import typedef_parser
...
class CompileTypeDefLinks(BaseCompile):
    def __init__(self, args: CompileLinkArgs) -> None:
        ...
        if self._do_sub:
            self._processer = str(Path(typedef_parser.__file__))
        else:
            self._processer = ''
        if self.args.path:
            self._process_path()
    ...
    def _subprocess(self, file: str):
        ...
        self.args.log.info(
            "CompileTypeDefLinks: Processing enums in file: %s", file)
