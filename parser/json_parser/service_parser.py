...
class ParserService:
    ...
    def get_links(self) -> List[urldata]:
        ...
        links: List[dict] = classes.get('service', [])
        for itm in links:
            name = itm['name']
            href = itm['href']
...
class WriterService:
    def __init__(self, parser: ParserService) -> None:
        ...
        self._proc = Path(self._parser.app_root, 'parser', 'service.py')
...
def main():
    ...
    p = ParserService(json_path=args.json_file)
    w = WriterService(parser=p)
    w.Write()
