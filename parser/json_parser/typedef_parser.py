...
import parser.typedef as typedef
...
class ParserTypeDef:
    ...
    def get_links(self) -> List[urldata]:
        ...
        enum_links: List[dict] = self._json_data['data'].get('typedef', [])
...
class WriterTypeDef:
    def __init__(self, parser: ParserTypeDef) -> None:
        self._parser: ParserTypeDef = parser
        self._proc = Path(self._parser.app_root, 'parser', 'typedef.py')
...
def parse(*args, **kwargs):
    ...
    p = ParserTypeDef(json_path=pkwargs['json_file'])
...
def main():
    ...
    p = ParserTypeDef(json_path=args.json_file)
    w = WriterTypeDef(parser=p)
    w.Write('t', 'j')
