...
class TblComponent(BaseSqlTable):
    ...
    def insert(self, data: List[Component]) -> None:
        ...
        VALUES (:id_component, :type, :version, :name, :namespace, :lo_ver, :file, :c_name)
        ON CONFLICT(id_component) 
        DO UPDATE SET type=excluded.type, version=excluded.version,
        name=excluded.name, namespace=excluded.namespace, lo_ver=excluded.lo_ver, file=excluded.file, c_name=excluded.c_name;
        """
