...
__CHEETAH_genTime__ = 1645650442.9079676
__CHEETAH_genTimestamp__ = 'Wed Feb 23 16:07:22 2022'
__CHEETAH_src__ = '_ex_dyn.tmpl'
__CHEETAH_srcLastModified__ = 'Wed Feb 23 16:07:20 2022'
...
if (not TYPE_CHECKING) and UNO_RUNTIME and UNO_ENVIRONMENT:
    ...
    def _get_class():
''')
        if VFFSL(SL,"is_sorted",True): # generated from line 52, col 1
            write('''        orig_init = None
        ordered_keys = (''')
            _v = VFN(VFFSL(SL,"self",True),"get_nt_names_str",False)() # '$self.get_nt_names_str()' on line 54, col 25
            if _v is not None: write(_filter(_v, rawExpr='$self.get_nt_names_str()')) # from line 54, col 25.
            write(''')
        def init(self, *args, **kwargs):
''')
            #  set $sorted_first = $self.get_dyn_constructor_args_lst(add_default=False)[0]
            write('''            if len(kwargs) == 0 and len(args) == 1 and getattr(args[0], "__class__", None) == self.__class__:
...
''')
        write("""        type_name = '""")
        _v = VFFSL(SL,"fullname",True) # '$fullname' on line 66, col 22
        if _v is not None: write(_filter(_v, rawExpr='$fullname')) # from line 66, col 22.
        write("""'
        ex = uno.getClass(type_name)
        ex.__ooo_ns__ = '""")
        _v = VFFSL(SL,"namespace",True) # '$namespace' on line 68, col 26
        if _v is not None: write(_filter(_v, rawExpr='$namespace')) # from line 68, col 26.
        write("""'
        ex.__ooo_full_ns__= type_name
        ex.__ooo_type_name__ = 'exception'
""")
        if VFFSL(SL,"is_sorted",True): # generated from line 71, col 1
            write('''        orig_init = ex.__init__
        ex.__init__ = init
''')
        write('''        return ex

    ''')
        _v = VFFSL(SL,"safe_name",True) # '$safe_name' on line 77, col 5
        if _v is not None: write(_filter(_v, rawExpr='$safe_name')) # from line 77, col 5.
        write(''' = _get_class()

else:
    ''')
        _v = VFN(VFFSL(SL,"self",True),"get_rel_import",False)(in_str=VFFSL(SL,"uno_obj_in",True), ns=VFFSL(SL,"uno_obj_ns",True)) # '$self.get_rel_import(in_str=$uno_obj_in, ns=$uno_obj_ns)' on line 80, col 5
        if _v is not None: write(_filter(_v, rawExpr='$self.get_rel_import(in_str=$uno_obj_in, ns=$uno_obj_ns)')) # from line 80, col 5.
        write(''' as ''')
        _v = VFFSL(SL,"safe_name",True) # '$safe_name' on line 80, col 65
        if _v is not None: write(_filter(_v, rawExpr='$safe_name')) # from line 80, col 65.
        write("""
